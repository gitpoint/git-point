export const ru = {
  auth: {
    login: {
      ...
      welcomeMessage: 'Самый многофункциональный клиент GitHub, который на 100% бесплатный',
      notificationsTitle: 'Управление уведомлениями',
      notificationsMessage: 'Просматривайте и контролируйте все ваши непрочитанных и участвующих уведомлений',
      reposTitle: 'Репозитории и пользователи',
      reposMessage: 'Легко получайте информацию о репозитории, пользователе и организации',
      issuesTitle: 'Задачи и Pull-запросы',
      issuesMessage: 'Общайтесь, сливайте pull-запросы и делайте многое другое',
      signInButton: 'ВОЙТИ',
    },
    welcome: {
      welcomeTitle: 'Добро пожаловать в GitPoint',
    ...
    events: {
      welcomeMessage: 'Добро пожаловать! Это ваша лента новостей - она поможет следить вам за последней активностью в репозиториях, которые вы отслеживаете, и за людьми, на которых вы подписаны.',
      ...
      pullRequestReviewEditedEvent: '{{action}} свой комментарий в pull-запросе',
      pullRequestReviewDeletedEvent: '{{action}} свой комментарий в pull-запросе',
    ...
    profile: {
      ...
      orgsRequestApprovalBottom: 'Возможно, вам придется запросить у них одобрение.',
    ...
    privacyPolicy: {
      ...
      introduction: 'Мы рады, что вы решили использовать GitPoint. Настоящая Политика конфиденциальности уведомляет вас о том, что мы делаем и не делаем с данными нашего пользователя.',
      userDataTitle: 'ДАННЫЕ ПОЛЬЗОВАТЕЛЯ',
      userData1: 'Мы никак не используем ващи данные на GitHub. После аутентификации токен пользователя OAuth сохраняется непосредственно в хранилище устройства. Мы не можем получить его. Мы никогда не просматриваем токен доступа пользователя и не храним его вообще.',
      userData2: 'Это означает, что мы никоим образом не можем использовать или передавать данные пользователя GitHub. Если его личные данные когда-либо становятся видимыми, мы не будем их записывать и просматривать. Если это случайно произошло, мы удалим их немедленно, используя безопасные методы удаления. Кроме того, мы специально настроили аутентификацию так, чтобы этого никогда не произшло.',
      analyticsInfoTitle: 'ИНФОРМАЦИЯ ОБ АНАЛИТИКЕ',
      analyticsInfo1: 'В настоящее время мы используем Google Analytics и iTunes App Analytics, чтобы измерить тенденции трафика и использования GitPoint. Эти инструменты собирают информацию, отправленную вашим устройством, включая версию устройства и платформы, регион и реферер. Этой информации недостаточно для идентификации какого-либо конкретного отдельного пользователя, никакая другая личная информация не передаётся.',
      analyticsInfo2: 'Если мы подключим другую стороннюю платформу для сбора трассировок стека, журналов ошибок или большей информации для аналитики, мы сделаем так, чтобы пользовательские данные оставались анонимными и зашифрованными.',
      openSourceTitle: 'ОТКРЫТЫЙ ИСХОДНЫЙ КОД',
      openSource1: 'GitPoint - это продукт с открытым исходным кодом, и история вкладов в приложение всегда будет видна публике.',
      openSource2: 'С каждым вкладом в приложение всегда выполняется проверка кода, чтобы никто не мог включить любого вида вредоносный код.',
      contactTitle: 'КОНТАКТЫ',
      contact1: 'Благодарим вас за ознакомление с нашей Политикой конфиденциальности. Мы надеемся, что вам понравится использовать GitPoint так же, как и нам нравится его разрабатывать',
      contact2: 'Если у вас есть какие-либо вопросы по поводу этой Политики конфиденциальности или GitPoint в целом, пожалуйста, создайте задачу в',
  ...
  repository: {
    main: {
      ...
      noContributorsMessage: 'Не найдено участников',
      sourceTitle: 'КОД',
      readMe: 'README',
  ...
  issue: {
    ...
    main: {
      ...
      mergeButton: 'Слить pull-запрос',
      noDescription: 'Нет описания.',
      lockedCommentInput: 'Заблокировано, но вы все ещё можете прокомментировать...',
